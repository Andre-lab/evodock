...
import numpy as np
from pyrosetta import Pose, Vector1, create_score_function
...
from src.genotype_converter import LocalGenotypeConverter
...
class LowResFitnessFunction:
    def __init__(self, native_pose, input_pose, trans_max_magnitude):
        ...
        self.scfxn_rosetta = create_score_function("interchain_cen")
        self.dock_pose = Pose()
        self.converter = LocalGenotypeConverter(native_pose)
    ...
    def get_dict_scores(self, pose):
        ...
        for e in energies_as_list:
            ...
            term = term.replace(";", "")
            if term in interchain_terms:
                weighted_val = float(unweighted_val) * interchain_terms[term]
