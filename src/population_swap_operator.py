...
import random
from pyrosetta import Vector1, Pose
        ...
        for ind in population:
            if random.uniform(0, 1) <= self.swap_prob:
                ...
                if after < ind.score:
                    ...
                    irms = self.dockmetric.i_rmsd(join_pose)
                    ...
                    self.accept_bb_into_ensemble(pose, idx_l, idx_r, idx_s, ind)
                    new_population.append(result_individual)
                    ratio_swap_success += 1
                    if relaxed:
                        relaxed_sucess += 1
                    if after < self.local_search.lowest_score:
                        self.local_search.lowest_score = after
